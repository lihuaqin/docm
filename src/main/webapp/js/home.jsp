...
function getServerOS() {
	$.ajax({
		...
		dataType : "text",
		data : {},
		url : "${basePath}/homeController/getServerOS.ajax",
...
function showFolderView(fid,targetId) {
	...
	$.ajax({
		...
		url : '${basePath}/homeController/getFolderView.ajax',
		success : function(result) {
			endLoading();
			if(result == "ERROR"){
				doAlert();
...
function dologin() {
	...
	if (check == "y") {
		...
		$.ajax({
			url : '${basePath}/homeController/getPublicKey.ajax',
			type : 'POST',
			data : {},
			dataType : 'text',
			success : function(result) {
...
function sendLoginInfo(encrypted) {
	...
	$.ajax({
		type : "POST",
		dataType : "text",
		url : "${basePath}/homeController/doLogin.ajax",
		data : {
			encrypted : encrypted,
			vercode : $("#vercode").val()
		},
		success : function(result) {
			...
			case "needsubmitvercode":
				$("#vercodebox").html("<label id='vercodetitle' class='col-sm-7'><img id='showvercode' class='vercodeimg' alt='点击获取验证码' src='${basePath}/homeController/getNewVerCode.do?s="+(new Date()).getTime()+"' onclick='getNewVerCode()'></label><div class='col-sm-5'><input type='text' class='form-control' id='vercode' placeholder='验证码……'></div>");
...
function getNewVerCode(){
	$("#showvercode").attr("src","${basePath}/homeController/getNewVerCode.do?s="+(new Date()).getTime());
}
...
function dologout() {
	...
	$.ajax({
		url:'${basePath}/homeController/doLogout.ajax',
		type:'POST',
		data:{},
		dataType:'text',
		success:function(result){
...
function createfolder() {
	...
	if (fn.length == 0) {
		...
		$.ajax({
			...
			url : "${basePath}/homeController/newFolder.ajax",
			success : function(result) {
				if (result == "mustLogin") {
					window.location.href = "prv/login.html";
				} else {
...
function deleteFolder(folderId) {
	...
	$.ajax({
		...
		url : "${basePath}/homeController/deleteFolder.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
function renameFolder(folderId) {
	...
	if (newName.length == 0) {
		...
		$.ajax({
			...
			url : "${basePath}/homeController/renameFolder.ajax",
			success : function(result) {
				if (result == "mustLogin") {
					window.location.href = "prv/login.html";
				} else {
	...
	if(isUpLoading==false && isImporting == false){
		if(fs!=null&&fs.length>0){
			...
			$.ajax({
				...
				url : "${basePath}/homeController/checkUploadFile.ajax",
				success : function(result) {
					if (result == "mustLogin") {
						window.location.href = "prv/login.html";
					} else {
...
function doupload(count) {
	...
	if (uploadfile != null) {
		...
		xhr.open("POST", "${basePath}/homeController/douploadFile.ajax", true);// 上传目标
...
function dodownload(fileId) {
	...
	window.location.href = "${basePath}/homeController/downloadFile.do?fileId=" + fileId;
}
...
function deleteFile(fileId) {
	...
	$.ajax({
		...
		url : "${basePath}/homeController/deleteFile.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
		...
		if (newFileName.length < 128) {
			if (!reg.test(newFileName) && newFileName.indexOf(".")!=0) {
				$.ajax({
					...
					url : "${basePath}/homeController/renameFile.ajax",
					success : function(result) {
						if (result == "mustLogin") {
							window.location.href = "prv/login.html";
						} else {
...
function showPicture(fileId) {
	$.ajax({
		url : "${basePath}/homeController/getPrePicture.ajax",
		data : {
			fileId : fileId
		},
		type : "POST",
...
function downloadAllChecked() {
	...
	// 计算预计耗时
	$.ajax({
		url:'${basePath}/homeController/getPackTime.ajax',
		type:'POST',
		data:{
	...
	$.ajax({
		type : "POST",
		url : "${basePath}/homeController/downloadCheckedFiles.ajax",
		data : {
			strIdList:faf.filesId,
			strFidList:faf.foldersId
		},
		...
		success : function(result) {
			...
			if (result == "ERROR") {
				...
				temp.action = '${basePath}/homeController/downloadCheckedFilesZip.do';
				temp.method = "post";
...
function deleteAllChecked() {
	...
	$.ajax({
		...
		url : "${basePath}/homeController/deleteCheckedFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
function playAudio(fileId) {
	...
	ap.list.clear();
	$.ajax({
		url:'${basePath}/homeController/playAudios.ajax',
		data:{
			fileId:fileId
		},
		type:'POST',
...
function doMoveFiles(){
	...
	$.ajax({
		...
		url : "${basePath}/homeController/confirmMoveFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
function sendMoveFilesReq(){
	...
	$.ajax({
		...
		url : "${basePath}/homeController/moveCheckedFiles.ajax",
		success : function(result) {
			if (result == "mustLogin") {
				window.location.href = "prv/login.html";
			} else {
...
function selectInCompletePath(keyworld){
	...
	$.ajax({
		...
		url : '${basePath}/homeController/sreachInCompletePath.ajax',
		success : function(result) {
			endLoading();
			if(result == "ERROR"){
				doAlert();
...
function ping(){
	$.ajax({
		url:"${basePath}/homeController/ping.ajax",
		type:"POST",
		dataType:"text",
		data:{},
		success:function(result){
...
function checkImportFolder(){
	if(isUpLoading == false && isImporting ==false){
		if(ifs != null && ifs.length > 0){// 必须选中文件
			...
			// 发送合法性检查请求
			$.ajax({
				url:'${basePath}/homeController/checkImportFolder.ajax',
				type:'POST',
				dataType:'text',
				data:{
...
function importAndCover() {
	$("#selectFolderImportModelAlert").hide();
	$.ajax({
		url:'${basePath}/homeController/deleteFolderByName.ajax',
		type:'POST',
		data:{
			parentId : locationpath,
			folderName : importFolderName
...
function importAndBoth() {
	...
	$.ajax({
		url:'${basePath}/homeController/createNewFolderByName.ajax',
		type:'POST',
		data:{
			parentId : locationpath,
			folderName : importFolderName,
...
function iteratorImport(i,newFolderName){
	...
	if (uploadfile != null) {
		...
		xhr.open("POST", "${basePath}/homeController/doImportFolder.ajax", true);// 上传目标
...
function sendChangePasswordInfo(encrypted){
	$.ajax({
		type : "POST",
		dataType : "text",
		url : "${basePath}/homeController/doChangePassword.ajax",
		data : {
			encrypted : encrypted,
...
function changePasswordGetNewVerCode(){
	$("#changepassword_showvercode").attr("src","${basePath}/homeController/getNewVerCode.do?s="+(new Date()).getTime());
}
...
function getFileChain(fileId,fileName){
	...
	$.ajax({
		type : "POST",
		dataType : "text",
		url : "${basePath}/homeController/getFileChainKey.ajax",
		data : {
			fid : fileId
		},
		success : function(result) {
